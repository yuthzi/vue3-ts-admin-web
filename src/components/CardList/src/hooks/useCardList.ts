...
export const useCardList = (
  ...
  const state = reactive<CardList.CardListStateProps>({
    // 列表数据
    cardData: [],
    // 分页数据
    pageable: {
  ...
  const getDataList = async () => {
    try {
      ...
      state.cardData = isPageable ? data.list : data
      // 解构后台返回的分页数据
   ...
   * @return void
   * */
  const updatePageable = (resPageable: CardList.Pageable) => {
